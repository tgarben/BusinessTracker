...
2. **Auto-Mileage (BETA): in or out of the 1.0 binary?** It requests **"Always" location** (App Review's most-scrutinized permission) AND is the least-tested code in the app (the 2026-06-24 election/lease/force-quit-recovery hardening has never run on a device or a real road). **Recommendation: pull it from 1.0** (ship manual + live-tracked mileage, which are solid and need no "Always"), release auto-mileage as 1.1 after road testing. If kept: serious on-device/road testing + an airtight Always-location justification string in App Review notes are mandatory.
...
- [ ] **CloudKit Dev → Production redeploy** — additive fields from the ⚠️ PENDING REDEPLOY list (see the iCloud Sync feature row): `Invoice/Quote.discountIsPercent`, `ExpensePreset.instantLog`, and (only if auto-mileage ships) `MileageTrip.isAutoDetected`/`needsReview`/`routePoints`. Run the shipping build on a device to register them in the Dev schema, then Deploy Dev → Production **before** the TestFlight/App Store build — or sync of those columns silently breaks for multi-device users. (The 2026-06-24 multi-device election is KV-store only — **no schema change**, no redeploy needed for it.)
