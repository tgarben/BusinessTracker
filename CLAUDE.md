...
| iCloud Sync | ✅ Done. Dev-environment sync confirmed on device 2026-06-13. **Production schema deployed 2026-06-17** (Development → Production via "Deploy Schema Changes…"), including all `CD_*` types plus `CD_Quote`/`CD_QuoteLineItem` — TestFlight/App Store builds now sync. **✅ Additive fields REDEPLOYED Dev → Production (2026-06-27)** — `Invoice.discountIsPercent`, `Quote.discountIsPercent`, `ExpensePreset.instantLog`, and `MileageTrip.isAutoDetected` + `needsReview` + `routePoints` are all live in Production. **⚠️ PENDING REDEPLOY:** `Invoice.includeTaxID` + `Quote.includeTaxID` (per-document Tax ID toggle, 2026-06-28), `MileageTrip.client` + inverse `Client.mileageTrips` (mileage→client link, 2026-06-28), and `MileagePreset.client` + inverse `Client.mileagePresets` (preset→client, 2026-06-29). All have safe defaults (`Bool = true` / optional relationship `= nil`) so the local store migrates fine, but they must be registered on-device then Dev → Production deployed before the next TestFlight/App Store build for CloudKit sync of the new columns/relationships. Reminder: any future new `@Model` or new field requires running the build on a device (to register it in the Development schema) then re-deploying Dev → Production before that build ships. |
...
- **`Client`** — `name`, `photoData: Data? = nil`, plus billing/customer fields `companyName`, `billingAddress`, `email`, `phone` (all `String = ""`, shown as "bill to" on invoices). Relationships: `projects` (cascade delete), `timeEntries` (nullify), `incomeEntries` (nullify), `expenses` (nullify), `mileageTrips` (nullify), `mileagePresets` (nullify), `timePresets` (nullify), `invoices` (cascade delete), `quotes` (cascade delete).
...
- **`MileageTrip`** — `date`, `startLocation`/`endLocation` (short display labels from `shortAddress`), `startAddress`/`endAddress` (full addresses for CSV export, captured via `fullAddress(_:)` at save), `miles`, `purpose`, `notes`, `waypoints: [String] = []` (intermediate stops, in order), **`client: Client? = nil`** (optional client link for per-client reports / tax — 2026-06-28; inverse `Client.mileageTrips`, nullify; set via a "Client" picker in `LogTripView` + `MileageTripEditView`, the latter being how auto/tracked trips get categorized). Computed `reimbursementAmount` using `ratePerMile` (IRS rate, currently 0.70), `allStops`, `routeDescription`, `startAddressForExport`/`endAddressForExport` (full address, else display label for older trips).
- **`MileagePreset`** / **`ExpensePreset`** — saved templates. MileagePreset: `name`, `startLocation`, `endLocation`, `purpose`, `notes`, `sortOrder`, **`client: Client? = nil`** (optional client link — inverse `Client.mileagePresets`, nullify; added 2026-06-29 so a recurring-route preset pre-fills the trip's client too). `ExpensePreset` is relationship-free. ExpensePreset: `name`, `amount: Double?` (nil = variable), `category`, `notes`, `sortOrder`, **`instantLog: Bool = false`** (per-preset — replaced the old global `expense_presetInstantSave` toggle, Jack's standup 2026-06-19; only applies to fixed-amount presets, edited via an "Instant-Log" toggle in `AddEditExpensePresetView` shown when Fixed Amount is on). Registered in the app Schema. Managed from Settings → Presets & Quick-Fill.
...
- **Timer sheet detents:** `TimerSheet` owns its own detents via the private `PhoneSheetDetents` modifier: **iPhone gets `[.medium, .large]`; iPad gets NO detents** so it presents as the standard centered form sheet like every other sheet (forcing `presentationDetents` on iPad produced an odd partial-height card). The three callers (`ContentView`, `HomeView`, `TimeTrackingView`) just present `TimerSheet()` with no detents of their own — don't re-add detents at the call site.
  - **Redesigned 2026-06-27 — fits at `.medium` without scrolling (no more auto-expand).** The old `syncDetent()`/`prefersLargeDetent` auto-jump-to-`.large` hack was removed: the idle layout is now compact enough to fit at `.medium`, so it just defaults to `.medium` and the user can drag to `.large` (extra room distributes via a `Spacer` above the pinned Start button). Three states, each its own layout helper: **idle** (`idleLayout` — compact 46pt clock + "Ready to track" caption, a single **`pickerCard`** combining Client + conditional Project as tappable `Menu`/`Picker` rows in one material card with a divider, a **`presetSection`** = "Quick Start" header + "Manage" + chip scroller (or a "Create a Preset" button when none), and a full-width green **Start Timer** button pinned at the bottom); **active** (`activeLayout` — big 64pt clock with a "Tracking"/"Paused" status label, client/project, full-width Pause-Resume + Stop split buttons); **saved** (`savedConfirmation`, centered via the `centered{}` helper). Picker rows use `Menu { Picker().pickerStyle(.inline) }` for a native checkmarked dropdown instead of the old heavy stacked material blocks. Clock falls back to `timerState.client`/`project` when `selectedClient`/`selectedProject` are nil (sheet opened on an already-running timer).
...
- **THREE ways to log a trip** (2026-06-21): (1) **manual entry** (the `+` FAB → `LogTripView`), (2) **live tracked trip** (the green play/`location.fill` FAB → `TripTracker`, the reliable battery-friendly GPS tracker), and (3) **automatic detection** (`DriveDetector`, opt-in Always-location background, Settings → Automatic Mileage). Manual + live-track ship enabled; auto is the off-by-default power-user option. The Mileage tab shows **two stacked FABs** (green Start Tracked Trip on top, blue manual `+` below) when not tracking.
  - **⚑ AUTO-MILEAGE IS SHELVED FOR 1.0** (Tyler + Jack, 2026-06-27). Only #1 (manual) and #2 (live tracked) ship. Auto-detection is **fully built but dormant** behind a single master switch **`DriveDetector.featureEnabled` (currently `false`)**: `refreshFromSettings()` early-returns (releasing any tracking-device election so other devices aren't blocked) and never starts monitoring, `setEnabled(_:)` no-ops, and the Settings "Automatic Mileage (BETA)" section is hidden (`if DriveDetector.featureEnabled` gate). No code was removed. To re-enable for 1.1: flip the flag to `true` and road-test (the auto fields `MileageTrip.isAutoDetected`/`needsReview`/`routePoints` are already deployed to CloudKit Production as of 2026-06-27, so no redeploy is needed unless you add *new* fields). The Mileage "Drives to Review" section is intentionally left in place (auto-hides when empty) so any leftover test drives stay reviewable/clearable; with detection off, no new ones appear.
...
- **Client link** (2026-06-28): a trip can be associated with a `Client` (optional `MileageTrip.client`). A "Client" picker (with a "None" option, hidden when no clients exist) appears in the Trip Details section of both `LogTripView` and `MileageTripEditView` — the editor is also how an auto-detected / live-tracked trip gets a client assigned after the fact. `TripRow` shows the linked client as a teal `person.fill` label. Flows to the per-client **Mileage section in `ClientDetailView`** (miles + reimbursement subtotal in the header) and a **Client column in the mileage CSV export**, and round-trips through `DataBackup`. Client soft-delete cascades to linked trips (like expenses).
...
- **Mileage presets:** route templates (`MileagePreset`). Chips at the top of the Distance section in `LogTripView` — tapping one pre-fills purpose/notes + start/end (and the optional **client**, 2026-06-29), then geocodes both endpoints (`CLGeocoder`) and auto-calculates driving distance; falls back to manual mode if geocoding fails. The optional client is set via a "Client" picker in `AddEditMileagePresetView` (Defaults section) and shown as a teal chip in the presets list. `applyPreset` only overwrites `selectedClient` when the preset carries one. Managed in Settings → Mileage Presets.
...
- **Mileage section** (2026-06-28): trips linked to this client (`@Query` on `MileageTrip.client == id`, excludes `needsReview`), shown via `TripRow`; header shows the client's total miles + reimbursement subtotal. Tap to edit (`MileageTripEditView`), swipe to delete with confirmation. Empty state nudges the user to link a trip from the Mileage tab. (Trips are linked from the Mileage tab, not created here.)
...
- **`AddEditClientView`:** photo picker at top (shows live avatar preview, initials fallback). Eagerly inserts a new `Client` into model context on appear for "New Client" flow — Projects section is available immediately so projects can be added before saving. **Cleanup runs in `.onDisappear` (`discardIfUnsaved()`), not the Cancel button** — so leaving without saving via EITHER the Cancel button OR swipe-to-dismiss deletes the eager-inserted client (a `didSave` flag set in `save()` prevents discarding a saved one; `workingClient = nil` guards double-run). Earlier the delete lived only in `cancel()`, so a swipe-dismiss left an empty client in the list (fixed 2026-06-28). Edit mode works identically.
...
- **Three info sources combine into each invoice:**
  1. **Your business info** — standardized in Settings → Business Information + Invoicing Defaults (`@AppStorage` keys `business_name`, `business_address`, `business_phone`, `business_email`, `business_website`, `business_taxID`, `business_defaultTaxRate`, `business_defaultPaymentTerms`, `business_acceptedPayments`, `business_paymentInstructions`). Rendered as the "from" header + tax ID + payment defaults.
     - **Per-document Tax ID toggle** (2026-06-28): `business_taxID` stays in Settings, but each invoice/quote has `includeTaxID: Bool = true` controlling whether the Tax ID / EIN prints in that document's PDF footer. The create form (`CreateInvoiceContent`/`CreateQuoteContent`) shows an "Include Tax ID / EIN" toggle (after Notes) **only when `business_taxID` is non-empty**. The PDF gates on `DocumentPDFSpec.showTaxID && !business.taxID.isEmpty` (a `showTax` local in `makeDocumentPDF`); `makeInvoicePDF`/`makeQuotePDF` pass `showTaxID: <doc>.includeTaxID`. The flag is carried through duplicate (`duplicateInvoice`/`duplicateQuote`) and quote→invoice conversion (`convertToInvoice`), and round-trips through `DataBackup` (optional `includeTaxID` DTO field, defaults `true` on import for older backups). No post-create edit UI (documents are finalized snapshots) — set it at creation.
...
- **Delete sites set `deletedDate = .now`** instead of `modelContext.delete(...)`. Exceptions that remain hard-delete: `TimePreset` (PresetsView), `Project` (AddEditClientView inline), and the eager-insert discard path in AddEditClientView (`discardIfUnsaved()` in `.onDisappear` hard-deletes an unsaved client).
...
Consolidated list to work through before App Store submission. (Reviewed 2026-06-24 — coding paused to focus on launch.)

### 🎯 Two launch-shaping decisions (make these first)
1. **Ship 1.0 FREE (recommended).** `Entitlements.monetizationEnabled = false` ⇒ the app already ships fully unlocked, no paywall. This is the de-risked path: it makes the entire monetization block below **non-blocking for 1.0** (defer to 1.1). Monetize later behind the existing `Entitlements`/`StoreProvider` seam. Only revisit if you decide to charge at launch.
2. **Auto-Mileage (BETA): in or out of the 1.0 binary?** ✅ **DECIDED 2026-06-27 — OUT.** Shelved for 1.0 via `DriveDetector.featureEnabled = false` (dormant, not removed — see the Mileage "AUTO-MILEAGE IS SHELVED" note). 1.0 ships manual + live-tracked mileage only, so **no "Always" location permission is exercised** and the least-tested code never runs. Revisit for 1.1 (flip the flag + road-test + CloudKit redeploy of the auto fields). _(Original reasoning, kept for context: it requested "Always" location — App Review's most-scrutinized permission — and was the least-tested code in the app.)_

### 🔴 Hard blockers for a FREE 1.0 (can't submit without)
- [ ] **Privacy Policy** — `PRIVACY.md` (repo root) still has placeholders: `[Month DD, YYYY]` effective date, legal entity name, contact email. Fill them, host it (Notion/site), link the URL in App Store Connect. Mandatory because of camera/location/photos.
- [ ] **App Store privacy nutrition labels** — declare camera, location, photos in App Store Connect.
- [ ] **Accessibility nutrition labels** (App Store Connect, per-platform) — claim status: ✅ **Dark Interface** + ✅ **Reduced Motion** (honored app-wide as of 2026-06-24, see Accessibility note below) are safe to claim now. 🧪 **VoiceOver / Voice Control / Larger Text / Differentiate Without Color Alone** — claimable after one on-device pass through the 5 core flows (log time/trip/expense, create invoice, read a report) at the largest Dynamic Type (AX5); watch fixed-width currency fields + badge pills for clipping. ❌ **Sufficient Contrast** — needs a contrast audit before claiming (`.secondary`/`.tertiary` captions + colored-on-dark badges are borderline). N/A: Captions, Audio Descriptions (no media).
- [ ] **CloudKit Dev → Production redeploy** — ✅ 2026-06-27 batch done (`Invoice/Quote.discountIsPercent`, `ExpensePreset.instantLog`, `MileageTrip.isAutoDetected`/`needsReview`/`routePoints`). **⚠️ Re-opened: `Invoice.includeTaxID` + `Quote.includeTaxID`, `MileageTrip.client`/`Client.mileageTrips` (2026-06-28), and `MileagePreset.client`/`Client.mileagePresets` (2026-06-29) still need the register-on-device-then-Deploy Dev → Production cycle before the next TestFlight/App Store build.** (The 2026-06-24 multi-device election is KV-store only — no schema change, never needed a redeploy.)
- [ ] **Real upgrade-path test** ⭐ — install the current App Store/TestFlight build, enter real data, upgrade to the 1.0 build, confirm nothing is lost. The #1 data-safety safeguard; extra important given recent schema additions. (See "Schema changes & data safety".)
- [ ] **App Store listing assets** — screenshots (incl. **iPad** sizes, since iPad is supported), description, keywords, **support URL**, marketing URL, age rating. Gate submission; easy to underestimate.

### 🟢 Done / no longer blocking (kept for reference)
- ✅ **Real app icon** — `AppIcon.appiconset` has the 1024 Default/Dark/Clear icons (`Freelanced-iOS-*-1024x1024@1x.png`). (The onboarding Welcome badge uses the separate `freelancedLogo` imageset; Live Activity badges are glyph-only — see that gotcha.)

### 🟡 Deferred to 1.1 — monetization (only when you decide to charge)
- [ ] **Wire a real `StoreProvider`** (StoreKit 2 or RevenueCat) + create products in App Store Connect (IDs `com.garbenTechnologies.BusinessTracker.pro.{monthly,annual,lifetime}`). Replaces `StubStoreProvider`.
- [ ] **Flip `Entitlements.monetizationEnabled` → `true`** (only after the real store layer is in).
- [ ] **On-device pass of the gated flows** post-flip (paywall → purchase → unlock; sandbox Apple ID in TestFlight).
- [ ] **Terms of Use (EULA) URL** for the paywall footer (Privacy Policy already covered above).
- [ ] **Paywall trial copy** — subscribe button shows "Start Free Trial" for monthly too; only claim a trial on plans actually configured for one (currently only annual).
- [ ] **Lock the free-client cap** — `Entitlements.freeClientLimit` (currently 2). Loosening later is easy; tightening causes revolts.
- [ ] **Decide monetization model** — freemium (current scaffolding) vs. whole-app subscription. Same `Entitlements`/`StoreProvider`/`PaywallView`; only gate *placement* changes.
- [ ] **Finish `GarbenApps.all`** (`Settings/MoreAppsView.swift`) — Sipfolio's public `testFlightURL` (+ later App Store URL) + confirmed taglines. (Cosmetic; non-blocking even for a free launch.)
...
- ✅ **Reduced Motion** (2026-06-24) — every custom animation in the app is gated on `@Environment(\.accessibilityReduceMotion)` via the `reduceMotion ? nil : <anim>` pattern (and `.contentTransition(reduceMotion ? .identity : .numericText())`). Default (Reduce Motion off) experience is unchanged; with it on, motion becomes instant. Covered: `ReportsView` (period stepper snappy + numeric label), `ExpensesView` (speed-dial FAB spring), `TimeTrackingView` (active-timer card spring), `TimerSheet` (save-confirm spring + timer numeric transition), `DocumentPreviewView` (created-toast spring/fade), `OnboardingView` (page transitions, progress-capsule spring, permission checkmark fades). **When adding any new animation, gate it the same way** to keep the Reduced Motion nutrition-label claim honest. Enables the App Store Connect accessibility claim (see pre-launch checklist).
...
- **Design:** app-icon-style badge (`TimerAppBadge` — a branded **gradient tile + `stopwatch.fill` SF Symbol** (glyph-only); the `freelancedLogo` image does NOT render in a Live Activity, see the GLYPH-ONLY gotcha below), a red-dot "TRACKING TIME" label (`TrackingLabel`), rounded monospaced indigo timer (`liveTimer`), indigo-tinted gradient container background. Brand colors `brandIndigo`/`brandIndigoDark` defined at top of file (`brandGradient` is now unused but kept).
...
- **Review UI** (`MileageView`): a **"Drives to Review"** section (orange `car.circle.fill` header) shows `isAutoDetected && needsReview` trips via a dedicated `@Query`; the normal day-grouped list uses `listedTrips` (= `trips.filter { !$0.needsReview }`) so review drives don't double-show. Per row, **three VISIBLE `.borderless` buttons** (2026-06-22 — swipe was undiscoverable): **Confirm** (green → `markReviewed`, clears `needsReview`), **Categorize** (blue → opens `MileageTripEditView`), **Delete** (red trash → soft-delete). Tapping the row also opens the editor. Confirm is the explicit "keep it" action.
  - **Collapsible** (2026-06-26): the section header is a tappable toggle (count badge + rotating chevron, Reduce-Motion-gated animation) so a long auto-detect backlog doesn't crowd the screen. State persists in `@AppStorage("mileage_reviewExpanded")` (default expanded; device-local, NOT iCloud-synced).
- **⚠️ Unreviewed auto-drives are EXCLUDED from every mileage total** (2026-06-26): a drive with `needsReview == true` must NOT count toward any miles/reimbursement/deduction figure until the user Confirms/Categorizes it (an unreviewed personal drive would otherwise inflate the numbers). Filtered out in: `MileageView.monthTrips`, `ReportsView.monthTrips` (drives all report mileage cards incl. IRS deduction + fuel analysis), `HomeView.reviewedTrips` helper (today's miles, YTD deduction, recent trips, recent-activity feed), and the `MileageHistoryView` / `MileageMonthDetailView` `@Query` predicates (history months + month detail). **Any NEW mileage aggregation must also `&& !$0.needsReview`** — the only place needs-review trips appear is the Drives-to-Review section.
...
- **Multi-device de-dup — tracking-device election (2026-06-24):** if auto-tracking is on for >1 device on the same iCloud account (e.g. iPhone + iPad both in the car) they'd each independently insert a `MileageTrip` for the same drive → CloudKit syncs both → double-logged. Fix (option 1): a **single elected tracking device** via `NSUbiquitousKeyValueStore` key **`mileage_autoDetectDeviceID`** (synced directly through the iCloud KV store — NOT via `CloudKeyValueSync`'s allow-list). The first device to enable claims it (writes its stable `localDeviceID`, persisted in UserDefaults key `mileage_localDeviceID` from `identifierForVendor`); only the owner runs a GPS session (`DriveDetector.isTrackingDevice` gates `start()`/`refreshFromSettings()`/`setEnabled()`). Other enabled devices stay **dormant** (no GPS, no battery) but still receive the logged trips via CloudKit and can review/categorize them — `DriveDetector.otherDeviceIsTracking` drives a Settings hint + a **"Use This Device Instead"** button (`makeThisTrackingDevice()` takes over the election). Devices react to remote claim/release via `NSUbiquitousKeyValueStore.didChangeExternallyNotification` → `refreshFromSettings()`. **No `@Model`/schema change** (election lives in the KV store, not SwiftData) → no CloudKit redeploy needed. No-iCloud fallback: `trackingDeviceID == nil` → `isTrackingDevice == true`, so a lone/offline device tracks normally. **The election is a LEASE (hardened 2026-06-24)** — see the edge-cases subsection: the owner stamps a heartbeat (`mileage_autoDetectHeartbeat`), `electionIsLive` requires a fresh heartbeat (< 72h), and `refreshFromSettings()` centralizes claim/heartbeat/release so a stale/absent owner is auto-taken-over and a non-`Always` owner releases rather than blocking everyone.
- **Sensitivity (2026-06-24):** `driveStartSpeed` lowered 6.0→**4.5 m/s** (≈13→10 mph instant-start) and `minMiles` 0.3→**0.2** to catch drives sooner / shorter trips. Trade-off: more false positives (passenger/short walks). The sub-`driveStartSpeed` motion-confirmation fallback (`wasDrivingRecently`) is unchanged. Both are `private let` heuristics at the top of `DriveDetector` — tune further from real road data.

#### Auto-Mileage edge cases — fixes & residuals (2026-06-24)
The edge cases catalogued earlier were addressed in a hardening pass. **⚠️ All of this is background-location behavior that could NOT be compile- or road-tested in this environment (command-line tools only) — verify each on a real device.** Grouped by area; ✅ = fixed, ⚠️ = residual/accepted.

**Tracking-device election (multi-device):**
- ✅ **Backup-restore device-ID clone.** `localDeviceID` now also stores the `identifierForVendor` it was minted under (`mileage_localDeviceVendorID`); if IFV changes (restored onto a different device), the ID is **re-minted** so two installs never share one election identity. (Skips re-mint while IFV is momentarily `nil` to avoid churn.)
- ✅ **Orphan / no failover / owner-lost-Always — all fixed by a LEASE.** Owner stamps `mileage_autoDetectHeartbeat` (epoch); `electionIsLive` requires both an owner AND a heartbeat younger than `leaseTimeout` (**72h**). `isTrackingDevice` = "I own it OR the election isn't live," so a dead/sold/uninstalled owner's stale lease lets any enabled+`Always` device **auto-take-over** on its next `refreshFromSettings()`. The owner re-stamps via `heartbeatIfOwner()` (throttled to ≤ once per `leaseTimeout/4` = 18h to spare the KV quota). `refreshFromSettings()` is now the **single policy choke point**: not-enabled → release+stop; enabled but not `Always` → **release** (so a downgraded owner can't block others) + stop + request; enabled+`Always` → claim/heartbeat + start. `setEnabled` and the auth-change delegate just call it.
- ✅ **Claim-race double-log → insert-time dedup backstop.** `buildTrip` calls `hasDuplicateAutoTrip(...)` before inserting: if a non-deleted auto trip within **±5 min** AND with **both endpoints < 300 m** already exists locally (the other device finalized first and it synced over), it skips. Conservative (requires time + both endpoints) so distinct trips are never merged. ⚠️ **Residual:** truly simultaneous finalizes (neither has synced yet) can still double — the unavoidable tail of a KV-lease design; the review UI shows both for the user to delete one.
- ⚠️ **"Use This Device Instead" copy.** With the lease, `otherDeviceIsTracking` (= enabled && not me && live) is now accurate, so the copy fits. The narrow stale-but-not-yet-expired window can still mislead briefly; acceptable.

**DriveDetector ↔ manual `TripTracker`:**
- ✅ **Reverse guard added.** `TripTracker.begin()` calls `DriveDetector.shared.cancelActiveDriveForManualTracking()`, which abandons (does NOT save) any in-progress auto-drive so a manually-started trip can't double-log the same drive. (The forward guard — auto won't start during a manual trip — was already there.)

**Background lifecycle:**
- ✅ **Finalize no longer depends on incoming fixes.** A wall-clock `finalizeTask` (`Task.sleep(stationaryTimeout)`, re-armed on movement, cancelled on finalize/teardown) finalizes a parked drive even if location callbacks stop entirely. The old in-`handle` staleness check is kept as an immediate path. *(Caveat: if the OS fully suspends the process, the Task is suspended too — the force-quit recovery below covers that.)*
- ✅ **Force-quit / purge mid-drive recovery.** The active drive is snapshotted to a Caches-dir JSON (`autodrive_inprogress.json`, throttled to ≤ every 15 s). On the next enabled launch, `recoverInterruptedDriveIfNeeded()` finalizes the snapshot **only if its last movement is older than `stationaryTimeout`** (drive clearly ended while dead) — a still-recent snapshot is discarded so the live session that continues the drive isn't double-logged. Gated on `modelContainer != nil` so it never deletes the snapshot before it can build the trip. ⚠️ **Residual:** up to ~15 s (throttle) + the start clip can still be missing; recovery is best-effort.
- ⚠️ **Start clip** (unchanged): the first sig-change can fire up to ~500m into a drive → the very start may be missing. Phase 3 origin-stitching territory.

**Sensitivity (after the 2026-06-24 bump) — accepted trade-offs, not bugs:**
- ⚠️ **More false positives** — `minMiles 0.2` + 10 mph start logs passenger rides, buses/trains, brisk walks near GPS drift, and bike rides as needs-review drives the user dismisses. Inherent to higher sensitivity; a future **sensitivity setting** (Phase 3) is the real lever.
- ⚠️ **Motion fallback unavailable** if Motion & Fitness is denied / no coprocessor → only the raw speed threshold starts a drive; slow starts (heavy traffic) may be missed on those devices. Can't be "fixed" without hurting precision.

**CloudKit / review data:**
- ⚠️ **Concurrent review edits** — categorizing on one device while deleting on another resolves by CloudKit last-writer-wins. Standard SwiftData+CloudKit behavior; low impact, left as-is.
...
- **Screenshot/demo seeder** (`Debug/ScreenshotSeed.swift`, DEBUG + simulator only): launch the app with the `--seed-screenshots` argument to wipe the sim's local store and load a fixed demo dataset (clients Clancy Brothers / Summit Cafe / Northgate Realty, time/mileage/expense/income entries, INV-001 + an overdue invoice, a Sent quote) and skip onboarding. Called from `BusinessTrackerApp.init()` via `ScreenshotSeed.runIfRequested`. No-op in release builds, on device, and without the flag. Build + run for App Store reference shots: `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild … -destination 'platform=iOS Simulator,id=<udid>' build` then `xcrun simctl install/launch booted … --seed-screenshots` and `xcrun simctl io booted screenshot`. (Note: if the sim is signed into a real iCloud account, `CloudKeyValueSync` may override the seeded `user_name`/avatar with the account's synced values.)
...
- **`AddEditClientView` eager insert:** for new clients, the `Client` object is inserted into the model context immediately on `onAppear` so the Projects section is available before saving. If the user leaves without saving, the client is deleted in **`.onDisappear` (`discardIfUnsaved()`)** so BOTH the Cancel button and swipe-to-dismiss clean up (guarded by a `didSave` flag). This means an abandoned "New Client" sheet briefly creates and then removes a record — expected behavior. ⚠️ **Don't move the discard logic back into `cancel()` alone** — that misses interactive swipe-dismiss and leaves an empty client in the list.
